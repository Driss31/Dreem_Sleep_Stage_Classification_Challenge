...
from utils.FFT import get_fft_values
from utils.PSD import get_psd_values
from utils.auto_correlation import get_autocorr_values
...
def get_features(x_values, y_values, mph, no_peaks=5):
    ...
    peaks_x, peaks_y = get_first_n_peaks(
        x_values[indices_peaks], y_values[indices_peaks], no_peaks
    )
...
def features_data(dataset, sensors):
    ...
    for sensor in sensors:
        ...
        x_train_, y_train_ = extract_features_labels(
            file_train_sensor, dataset["y"], t, n, f_s, denominator=10
        )
...
def gene_features(hdfs, nbr):
    ...
    for key in tqdm.tqdm(list(hdfs)[:11]):
        ...
        df_train["Entropy %s" % key] = np.apply_along_axis(
            calc.approximate_entropy, 1, temp, *args
        )
